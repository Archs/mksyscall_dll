...
/*
mksyscall_dll generates windows system call bodies
...
func ParseFiles(fs []string) (*Source, error) {
	...
	basename := filepath.Base(fs[0])
	ns := strings.Split(basename, ".")
	src.PkgName = ns[0]
	return src, nil
}
...
{{define "main"}}// mksyscall_dll{{range .Files}} {{.}}{{end}}
...
{{define "dlls"}}{{range .DLLs}}	mod{{.}} = syscall.NewLazyDLL("{{.}}.dll")
{{end}}{{end}}
